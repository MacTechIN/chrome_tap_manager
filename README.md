...
| `../Chrome_Window_manager/` (별도 폴더·별도 git) | ext-v0.3.0 기준 독립 배포 스냅샷 "Chrome Window Manager" (지인 테스트용) |
...
| Extension 구현 | 🟢 E00~E12 완료 (EM6) | `ext-v1.0.0`. 남은 것: 실기기 검증, 스토어 등록 |
...
| 2026-09-12 | (ext-v0.3.0에 포함) | `455b5fb` | E07 Omnibox(`t` + 키워드, `<match>` 하이라이트, Enter로 탭/주제 전환) + **검색 무응답 수정**(manifest에 omnibox가 없으면 SW가 시작 중 죽어 메시지 리스너 미등록 → 방어 코드, 팝업에 원인 표시). 테스트 196건 |
