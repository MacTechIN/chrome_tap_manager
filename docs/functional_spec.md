...
- 상태: 기술 스택 선정 완료, **정리 모델 "창 = 주제" 확정(v0.2)**. 상세 UI 시안·API 스키마는 후속 문서에서 확정
...
3. **자동화**: 검색창에서 주제를 통째로 열기 / 닫기, 반복 행동을 규칙으로 학습

### 1.5 정리 모델 (v0.2 확정): 창 하나 = 주제 하나
...
| 창을 닫아도 주제는 남음 | 닫힌 창은 "저장된 주제"가 되어 검색창에서 이름을 치면 통째로 다시 열린다 |
...
| 용어 | 정의 |
|---|---|
| **Topic (주제)** | **하나의 일반(type=normal) Chrome 창에 1:1로 대응하는 작업 단위.** 이름, 색상, 선택적 URL 규칙을 가진다. 창이 열려 있으면 `open`, 창이 닫히면 탭 목록을 보존한 채 `saved` 상태가 된다 |
| **Topic Window** | Topic에 대응하는 Chrome 창. `open` 상태의 Topic은 항상 정확히 하나의 창을 가진다 |
...
| **Saved Tab (저장 탭)** | `saved` 상태 Topic에 기록된 탭, 또는 `open` Topic에서 닫혔지만 보존 옵션으로 남은 탭 (URL, title, favicon) |
...
| F-04 | 창 ↔ Topic 매핑 유지 (창 생성/닫힘/병합 추적, 저장 상태 전환) | EXT | P0 |
...
| F-09 | 검색창에서 Topic 통째로 열기 / 닫기(저장) | APP, EXT | P1 |
...
- **삭제**: `saved` Topic만 삭제 가능 (저장 탭 목록 포함, 확인 필요). `open` Topic은 창을 닫으면 `saved`로 바뀔 뿐 삭제되지 않는다
- **설명(선택)**: 검색 대상에 포함

### F-02 탭을 Topic으로 보내기 / 새 Topic으로 분리 (P0)
- **기본 경로 (검색창)**: 검색창에 `>move <주제>` 또는 `>` 모드에서 주제 이름 선택 + Enter → 현재 활성 탭을 그 Topic Window로 `tabs.move`. 대상이 `saved` Topic이면 창을 새로 열어(`windows.create`) 저장 탭과 함께 복원한 뒤 이동
...
- `windows.onRemoved` → 해당 Topic을 `saved`로 전환하고 마지막 탭 목록을 저장 탭으로 보존 (탭이 0개였으면 Topic도 삭제 여부를 설정으로 결정, 기본: 임시 이름 Topic은 삭제, 이름 지정 Topic은 보존)
- 탭이 다른 창으로 옮겨지면(`tabs.onAttached`) 그 탭의 Topic도 대상 창으로 바뀐다
- 창의 마지막 탭이 다른 창으로 옮겨져 창이 사라지는 경우도 `onRemoved`와 동일 처리
- Chrome 재시작 시 F-12가 `saved` Topic과 새 창들을 지문으로 대조해 매핑을 복구
...
### F-07 키워드 검색 (P0)
- 검색 대상: 열린 탭(title, URL host/path), Topic 이름·설명, 하위 그룹 이름, 저장 탭, `saved` Topic
- 매칭:
...
- 시퀀스:
  ...
  4. 런처 숨김
- `saved` Topic / 저장 탭 선택 시: 창을 새로 열어 복원(`windows.create` + `tabs.create`) 후 동일 시퀀스
- 실패 처리: EXT 미연결 시 결과 행에 "Chrome 연결 필요" 배지, Enter 시 연결 안내

### F-09 검색창에서 Topic 통째로 열기 / 닫기 (P1)
- `>open <topic>`: `saved` Topic을 새 창으로 복원 (저장 탭 전체 오픈). 이미 `open`이면 전면화
- `>close <topic>`: 창을 닫고 `saved`로 전환 (F-04 규칙, 확인 필요)
- `>save`: 현재 창의 탭 목록을 지금 시점으로 저장 탭에 스냅샷 (창은 유지)
- `>merge <topic>`: 현재 창의 탭 전부를 대상 Topic 창으로 이동 후 현재 창 닫기 (현재 Topic은 saved로 남지 않고 삭제, 확인 필요)
...
- **규칙 적용**: `tabs.onUpdated`에서 URL 확정 시 규칙 평가 → 첫 매칭 Topic의 창으로 `tabs.move`. 현재 창이 이미 그 Topic이면 아무것도 하지 않음. 대상 Topic이 `saved`면 자동 복원하지 않고 배지로만 알림(설정으로 자동 복원 선택 가능)
...
- **Side Panel 개요 (P1)**: Topic(창) 목록 → 하위 그룹 → 탭 트리. 드래그로 탭을 다른 Topic으로 이동, Topic 이름/색상 편집, 규칙 편집, `saved` Topic 목록. 정리의 **보조** 화면이며 필수 경로가 아니다

### F-12 세션 저장 / 복원 (P1)
- APP SQLite(연결 시) 및 EXT `storage.local`에 Topic → 탭 스냅샷을 항상 유지
- Chrome 재시작 후 EXT `onStartup`에서: 새로 열린 각 창의 탭 지문 집합과 `saved`/직전 `open` Topic의 탭 지문 집합을 비교(Jaccard ≥ 0.5)하여 매핑 복구 → Topic 이름·색상·하위 그룹 복원. 매칭 실패 창은 새 Unnamed Topic
- 탭 닫힘 시 저장 탭으로 보존하는 옵션 (기본 OFF: 저장 탭이 빠르게 늘어나는 것을 방지. `>save`와 창 닫힘은 항상 보존)
- JSON 내보내기/가져오기 (F-15)
...
| 테이블 | 주요 컬럼 |
|---|---|
| `topic` | `id`, `name`, `is_named`(사용자 지정 여부), `color`(nullable), `description`, `status`(`open`/`saved`), `window_id`(nullable, 휘발, `open`일 때만), `browser`, `last_active_at`, `created_at`, `updated_at` |
...
| APP→EXT | `open` | `{topicId}` (saved → 창 복원) |
